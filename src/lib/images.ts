...
export const IMAGES = {
  ...
  faculty_ithod: `${P}/college/ithod.png`,

  // Alumni — correctly in /faculty/
...
export const EVENT_IMAGE_MAP: { keywords: string[]; image: string }[] = [
  { keywords: ["techfest", "technical festival", "oscillations"],  image: IMAGES.event_techfest },
  { keywords: ["hacknova", "hackathon", "hack"],                   image: IMAGES.event_hackathon },
  { keywords: ["odyssey", "cultural festival", "annual cultural"], image: IMAGES.event_odyssey },
  { keywords: ["shiv jayanti"],                                    image: IMAGES.event_shiv_jayanti },
  { keywords: ["convocation", "degree", "certificate"],           image: IMAGES.event_convocation },
  { keywords: ["sports week", "sports", "krida"],                  image: IMAGES.event_sports_week },
  { keywords: ["blood donation"],                                  image: IMAGES.event_blood_donation },
  { keywords: ["tree plantation", "plantation"],                   image: IMAGES.event_tree_plant },
  { keywords: ["cloud", "workshop", "mastering"],                  image: IMAGES.event_cloud_workshop },
  { keywords: ["algorhythm", "aces"],                              image: IMAGES.event_algorhythm },
  { keywords: ["science day", "national science"],                 image: IMAGES.event_science_day },
  { keywords: ["csi", "project competition"],                      image: IMAGES.event_csi_project },
  { keywords: ["poster presentation"],                             image: IMAGES.event_poster_presentation },
  { keywords: ["alumni", "expert talk", "guest lecture"],         image: IMAGES.event_alumni_talk },
  ...
  { keywords: ["republic day", "independence"],                    image: IMAGES.event_republic_day },
  { keywords: ["industrial visit", "visit"],                       image: IMAGES.event_industrial_visit },
  { keywords: ["add-on", "addon", "embedded"],                     image: IMAGES.event_addOn },
];

export function normalizeImageUrl(url: string | null | undefined): string | null {
  if (!url || typeof url !== "string") return null;
  // Fix double images/images/ nesting if any old DB records still have it
...
export function getAchievementImage(title: string): string {
  ...
  if (lower.includes("nec") || lower.includes("iit bombay") || lower.includes("entrepreneurship"))
    return IMAGES.achievement_nec_iit;
  if (lower.includes("avishkar"))
    return IMAGES.achievement_avishkar;
  ...
  if (lower.includes("hackathon"))
    return IMAGES.achievement_internal_hackathon;
  const pool = [
