...
const DEFAULT_SUMMARY = { placements: 0, avgPackage: "0", research: 0, patents: 0, events: 0, achievements: 0, faculty: 0 };
...
export async function getDeptSummary(dept_id: string) {
  ...
  const [placements, research, patents, events, achievements, faculty] = await Promise.all([
    ...
    supabase.from("achievements").select("id", { count: "exact" }).eq("dept_id", dept_id).eq("status", "approved"),
  ...
  return { placements: placements.count ?? 0, avgPackage: avgPkg, research: research.count ?? 0, patents: patents.count ?? 0, events: events.count ?? 0, achievements: achievements.count ?? 0, faculty: faculty.count ?? 0 };
}
